...
namespace toy3d
{
	...
	void Renderer::DrawRect(const Rect& rect, Texture& texture)
	{
		...
		cmdBufs_[curFrame_].pushConstants(layout, vk::ShaderStageFlagBits::eFragment, sizeof(Mat4), sizeof(Color), &drawColor_);
		cmdBufs_[curFrame_].drawIndexed(6, 1, 0, 0, 0);
	}

	void Renderer::DrawLine(const Vec2d& p1, const Vec2d& p2)
	{
		auto& ctx = Context::GetInstance();
	...
	void Renderer::bufferRectVertexData()
	{
		Vertex2d vertices[] =
		{
			{Vec2d{-0.5, -0.5},	Vec2d{0, 0}},
			{Vec2d{0.5, -0.5},	Vec2d{1, 0}},
			{Vec2d{0.5, 0.5},		Vec2d{1, 1}},
			{Vec2d{-0.5, 0.5},	Vec2d{0, 1}},
		};
	...
	void Renderer::bufferLineData(const Vec2d& p1, const Vec2d& p2)
	{
		Vertex2d vertices[] =
		{
			{p1,	Vec2d{0, 0}},
