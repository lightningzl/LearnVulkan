...
namespace toy3d
{
	std::vector<vk::VertexInputAttributeDescription> Vec2d::GetAttributeDescription()
	{
	...
	std::vector<vk::VertexInputBindingDescription> Vec2d::GetBingDescription()
	{
	...
	Mat4 Mat4::CreateOrtho(int left, int right, int top, int bottom, int near, int far)
	{
		...
		mat.Set(3, 0, (float)(left + right) / (left - right));
		mat.Set(3, 1, (float)(top + bottom) / (bottom - top));
		mat.Set(3, 2, (float)(near + far) / (far - near));

		return mat;
	}

	Mat4 Mat4::CreateTranslate(const Vec2d& pos)
	{
		Mat4 mat = CreateIdentity();
		mat.Set(3, 0, pos.x);
		mat.Set(3, 1, pos.y);
	...
	Mat4 Mat4::CreateScale(const Vec2d& scale)
	{
		Mat4 mat = CreateIdentity();
		mat.Set(0, 0, scale.x);
		mat.Set(1, 1, scale.y);
